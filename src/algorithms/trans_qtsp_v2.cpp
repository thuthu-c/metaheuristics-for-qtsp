...
void TransQTSPV2::individualCostRank(std::vector<Individual>& population) {
    std::vector<Individual*> copyPopulation;
    for (auto& i : population) copyPopulation.push_back(&i);
...
void TransQTSPV2::evaluatePopulation(std::vector<Individual>& population) {
...
TransQTSPV2::TransQTSPV2(int maxEvaluations, int populationSize, double probT, double stepProb, double plasmidSize, int plasmidBankSize) 
    : TransQTSPProbT(maxEvaluations, populationSize, probT, stepProb, plasmidSize) 
{
    ...
    this->plasmidBankSize = plasmidBankSize;
    this->uElite=10.0;
}
...
std::vector<Individual> TransQTSPV2::generate_population(Graph *graph){
    ...
    // como vou verificar se há individuos repetidos na populacao? 
    // verificar individuos com mesmo custo

    // std::cout<< "Vamos printar a populacao? " << std::endl;
    // for(auto p : population){
    //     std::cout<< "a tour eh "<< std::endl;
    //     for(auto i : p.tour) std::cout<< i << " "; 
    //     std::cout<< std::endl;
    //     std::cout<< " o custo dessa tour eh " << p.cost;
    //     std::cout<< std::endl; 
    // }
...
std::vector<int> TransQTSPV2::run(Graph& graphInput){
    ...
    auto atualizarGirBiased = [&]() {
        ...
        if(!girConverted.empty()){
            for(auto i{0}; i < plasmidBankSize; ++i){
